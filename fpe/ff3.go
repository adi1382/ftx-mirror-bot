...
// A cipherFF3 is an instance of the FF3 mode of format preserving encryption
// using a particular key, radix, and tweak
type cipherFF3 struct {
	tweak  []byte
	radix  int
	minLen uint32
	maxLen uint32
...
// newCipher initializes a new FF3 cipherFF3 for encryption or decryption use
// based on the radix, key and tweak parameters.
func newCipher(radix int, key []byte, tweak []byte) (cipherFF3, error) {
	var newCipher cipherFF3
...
func (c cipherFF3) Encrypt(X string) (string, error) {
	return c.EncryptWithTweak(X, c.tweak)
}

// EncryptWithTweak is the same as Encrypt except it uses the
// tweak from the parameter rather than the current cipherFF3's tweak
// This allows you to re-use a single cipherFF3 (for a given key) and simply
// override the tweak for each unique data input, which is a practical
// use-case of fpe for things like credit card numbers.
func (c cipherFF3) EncryptWithTweak(X string, tweak []byte) (string, error) {
	var ret string
	var ok bool
...
func (c cipherFF3) Decrypt(X string) (string, error) {
...
// override the tweak for each unique data input, which is a practical
// use-case of fpe for things like credit card numbers.
func (c cipherFF3) DecryptWithTweak(X string, tweak []byte) (string, error) {
	var ret string
	var ok bool
